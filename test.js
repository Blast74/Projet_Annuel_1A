var numberOfSelectedMusics = 0; //Nombre de musiques trouvées pour une catégorie séléectionnée
var currentPage = 1; //Page sur laquelle est l'utilisateur
var lastPageMusicNumber =0;  //Nombre de musiques sur la dernières page
var LastPage =0; //n° de la dernières page
...
function CountPages () {
	if (numberOfSelectedMusics <5) {
		LastPage =1;
		lastPageMusicNumber = numberOfSelectedMusics;
	}
	else {
		LastPage = (((numberOfSelectedMusics-(numberOfSelectedMusics%5))/5)+1); //Nombre de pages
		lastPageMusicNumber = numberOfSelectedMusics%5; //Nombre de musiques pour la dernière page
	}
}
...
function refreshPages () { // il faut ajuster la taille en fonction des suppressions de contenu
	...
	if (LastPage == 1) {
		...
		container.innerHTML = "Page n°"+currentPage+" / "+LastPage;
	}
...
function previous () {//Page précédente //VERIFIER SI IL EXISTE OU PAS
	...
	if (currentPage == LastPage){ //Créer le bouton suivant si on est plus sur la dernières page
		container = document.getElementById('nextContainer');
...
function next () {//Page suivante
	...
	if (currentPage == (LastPage-1)) { //VERIFIER SI IL EXISTE OU PAS
		var removeButton;
		container = document.getElementById('nextContainer');
...
function selection () {
	...
	numberOfSelectedMusics = musicList[selection.value].length; //A REMPLACER PAR LE CONTENU DE LA BDD
	currentPage=1;
	CountPages();
...
function playersDisplay (action) {
	...
	if (action == 'reduce') {
		...
		//etoiles (note)
		//A VIRER
		console.log ('Elements à afficher: '+lastPageMusicNumber);
		//supprime les balises audio en fonction du nombe de musique à aficher sur la page
		for (var i = 5; i > lastPageMusicNumber; i --){
...
function playerContent (musicStyle, currentPage) {//changements de page à faire
	...
	if (LastPage == 1) { //S'il n'y a qu'une page à afficher
		playersDisplay('reduce');
		for (var i = 0; i < lastPageMusicNumber; i++) {
		...
		if (currentPage == LastPage){//Si c'est la dernières page
